...
pReact && define && (define("promise", ["pReact"], function() {
	...
	return {
		...
		readattrs: function(dom, parentNode) {
			...
			var vdom = {
				...
				setState: function(options) {
					pReact.extend(this, options);
					pReact.renderDom(this.pReactClass, this.state, this.parent, this.pReactClassCallback, true);
					this.update();
					return this;
				},
				update: function() {
		...
		diffDom: function(id, data, parent, bool) {
			//console.log(parent)
			var that = this;
			var dom = typeof id != "string" ? id : document.getElementById(id);
			//console.log(dom.vdom)
			if (!dom.vdom && bool) {
				var vdom = that.readattrs(dom, parent || dom.parentNode);
				data.push(vdom);
			}
			var diff = false,
				diffParent = null,
			...
			pReact.each(data, function(i, item) {
				...
				if (r.diff){
					...
					diffIndex = r.index;
					diffParent = r.parent;
					!dom.vdom && (dom.vdom = item.childNodes[r.index]);
					return false;
				}
			});
			return {
	...
	$.extend($, {
		...
		renderDom: function(html, data, parent, callback, bool) {
			...
			$.promise.when(function(resolve, reject) {
				...
				a && (pReact.each(a.children, function(i, item) {
					...
					r.diff && pReact.extend(pReact.vdoms[r.index], {
						...
						pReactClass: obj,
						parent: parent,
						pReactClassCallback: callback
					});
				}), !bool ? parent.appendChild(a) : ((parent.innerHTML = ""), parent.appendChild(a)));
...
pReact && (function(window, document, Math, $) {
  ...
  */
	...
	IScroll.prototype = {
		...
		refresh: function() {
			...
			this.maxScrollY = this.wrapperHeight - this.scrollerHeight - Math.abs(this.options.startY);
 ...
 *
 * https://github.com/ereddate/pReact
 * <videoplay src="video.mp4"></videoplay>
 */
 pReact && (pReact.video = function(callback) {
	var xvideo = function() {
	...
	xvideo.fn = xvideo.prototype = {
		...
		ready: function(callback) {
			...
			doms.length > 0 && ((styles = document.createElement("style")),header.appendChild(styles),(styles.innerHTML = 'videoplay{background:rgba(0,0,0,.8);display:block;position:relative;}videoplay.fullscreen{background:rgba(0,0,0,.8);display:block;position:absolute;top:0;left:0;right:0;bottom:0;z-index:999999;}videoplay.fullscreen video{height:100%;}videoplay span.videostatus{position:absolute;top:10px;left:10px;padding:0 5px;color:rgba(255,255,255, .5);}videoplay span.videostatus.hide{display:none;}videoplay span.netstate{position:absolute;top:10px;right:10px;padding:0 5px;color:rgba(255,255,255, .5);}'));
